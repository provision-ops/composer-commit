...
class Plugin implements PluginInterface, Capable, EventSubscriberInterface
{
    ...
    {
        return array(
          'Composer\Plugin\Capability\CommandProvider' => 'ProvisionOps\ComposerCommit\CommandProvider',
        );
    }

    /**
     * Run before Composer Update and Composer Require.
     */
    protected function preCommand() {
        if ($this->gitRepo->isDirty()) {
            return $this->io->writeError('Git working copy is not clean. Remove them before continuing. Run "git status" to view changes. Try "git reset --hard" to wipe out all changes.');
        }
    }

    /**
     * Run after Composer Update and Composer Require.
     */
